...
func TestVerifyAI_ProductionPath_Attack(t *testing.T) {
	...
	if result.RiskScore == 50 { t.Skip("Ollama unreachable") }
	if result.RiskScore < 80 {
		t.Errorf("Expected RiskScore >= 80, got: %d", result.RiskScore)
	}
	if result.Decision != "BLOCK_IP" {
		t.Errorf("Expected BLOCK_IP, got: %s", result.Decision)
	}
}
...
func TestVerifyAI_ProductionPath_Benign(t *testing.T) {
	...
	if result == nil {
		t.Fatalf("Expected result on 3rd benign event")
	}
...
func TestVerifyAI_ProductionPath_CrossTool(t *testing.T) {
	...
	if result == nil {
		t.Fatalf("Expected AI trigger on High event")
	}
	if result.RiskScore == 50 { t.Skip("Ollama unreachable") }
